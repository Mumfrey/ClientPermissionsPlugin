...
public class ReplicatedPermissionsMonitor
{
	...
	{
		playerModInfo.put(player.getName(), new Hashtable<String, ReplicatedPermissionsContainer>());
	}
	...
	{
		playerModInfo.remove(player.getName());
	}
	...
	public void onQuery(Player player, ReplicatedPermissionsContainer query)
	{
		if (query != null)
		{
			if (!playerModInfo.containsKey(player.getName()))
			{
			...
			playerModInfo.get(player.getName()).put(query.modName, query);
